...
def find_cover_image(article_dir, cover_arg=None):
    ...
    # 回退到仓库根目录的默认封面目录
    root_cover_dir = SKILL_DIR / "covers"
    if root_cover_dir.exists():
        for ext in ("cover_*.png", "cover_*.jpg", "cover_*.jpeg"):
            covers = sorted(root_cover_dir.glob(ext))
            if covers:
                return covers[0]
...
def main():
    ...
    if args.input:
        ...
        theme = args.theme
        if not theme:
            if SELECTED_THEME_FILE.exists():
                saved = SELECTED_THEME_FILE.read_text(encoding="utf-8").strip()
                if saved:
                    theme = saved
        ...
        article_dir = OUTPUT_DIR / file_stem
    else:
