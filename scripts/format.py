...
SELECTED_THEME_FILE = Path(CONFIG.get("selected_theme_file", str(OUTPUT_DIR / "selected-theme.txt")))
...
def convert_lists_to_sections(html: str, style_map: dict, depth: int = 0) -> str:
    ...
    def append_inline_content(target: Tag, source: Tag):
        for child in list(source.contents):
            if isinstance(child, Tag) and child.name in ("ul", "ol"):
                continue
            if isinstance(child, Tag) and child.name == "p":
                for grandchild in list(child.contents):
                    target.append(grandchild.extract())
            else:
                target.append(child.extract())

    def build_list_section(list_tag: Tag, current_depth: int) -> Tag:
        indent = f"padding-left:{16 * current_depth}px;" if current_depth > 0 else ""
        wrap = soup.new_tag("section")
        wrap["style"] = f'{wrapper_style}{indent}' if indent else wrapper_style

        items = [child for child in list_tag.children if isinstance(child, Tag) and child.name == "li"]
        for idx, li in enumerate(items, 1):
            bullet = str(idx) if list_tag.name == "ol" else "•"
            bullet_s = ol_bullet_style if list_tag.name == "ol" else bullet_style

            row = soup.new_tag("section")
            row["style"] = f"{row_style}{indent}"

            bullet_span = soup.new_tag("span")
            bullet_span["style"] = bullet_s
            bullet_span.string = bullet

            text_span = soup.new_tag("span")
            text_span["style"] = text_style
            append_inline_content(text_span, li)

            row.append(bullet_span)
            row.append(text_span)
            wrap.append(row)

            nested_lists = [
                child for child in list(li.contents)
                if isinstance(child, Tag) and child.name in ("ul", "ol")
            ]
            for nested in nested_lists:
                wrap.append(build_list_section(nested, current_depth + 1))
    ...
    for list_tag in root_lists:
        list_tag.replace_with(build_list_section(list_tag, depth))
...
def convert_image_captions(html: str) -> str:
    ...
    for p in soup.find_all("p"):
        ...
        prev_has_image = (
            (prev.name == "section" and prev.find("img") is not None)
            or (prev.name == "p" and prev.find("img") is not None)
        )
        ...
        p.string = only_child.get_text(strip=True)
...
def generate_gallery(rendered_map: dict, theme_map: dict,
    ...
    # 写入选中主题到配置指定的位置（默认第一个）
    SELECTED_THEME_FILE.parent.mkdir(parents=True, exist_ok=True)
    SELECTED_THEME_FILE.write_text(default_theme, encoding="utf-8")
...
def main():
    ...
    if CONFIG_PATH.name == "config.example.json":
        raise SystemExit("请先复制 config.example.json 为 config.json，再运行 scripts/format.py")
    ...
    if args.gallery:
        ...
        print(f"选中的主题 ID 会写入 {SELECTED_THEME_FILE}")
        return
