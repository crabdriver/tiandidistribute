...
def _get_cfg(cfg: dict) -> tuple[str, dict, dict]:
    ...
    if not settings.get("base_url") and ai.get("url"):
        settings = {**settings, "base_url": ai["url"]}
    elif not settings.get("base_url") and os.environ.get("OPENROUTER_URL"):
        settings = {**settings, "base_url": os.environ["OPENROUTER_URL"]}

    if not settings.get("model") and ai.get("model"):
        settings = {**settings, "model": ai["model"]}

    if not secrets.get("api_key") and ai.get("api_key"):
        secrets = {**secrets, "api_key": ai["api_key"]}
    elif not secrets.get("api_key") and os.environ.get("OPENROUTER_API_KEY"):
        secrets = {**secrets, "api_key": os.environ["OPENROUTER_API_KEY"]}
...
def main():
    ...
    using_example_fallback = args.config is None and not config_path.exists() and (skill_dir / "config.example.json").exists()
    cfg = _load_config(config_path)

    if using_example_fallback:
        raise SystemExit("请先复制 config.example.json 为 config.json，再运行 scripts/generate.py")
