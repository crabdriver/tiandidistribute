...
def get_page_text_snippet(target_id, limit=2000):
    ...
    return run_live_cdp("eval", target_id, expression)
...
def run_preflight_checks(platforms, mode, workbench):
    ...
    if "wechat" in platforms:
        ...
        if not wechat["covers_ready"]:
            blockers.append("微信公众号缺少封面图目录 `covers/` 或可用的 `cover_*.png` 文件")
...
def run_platform(platform, markdown_file, mode):
...
def bind_workbench(platforms, tabs):
    ...
    for platform in platforms:
        if platform not in BROWSER_PLATFORMS:
            continue
        existing_target = existing.get(platform)
        if existing_target and existing_target in live_targets:
            bound[platform] = existing_target
            continue

        target = find_platform_target(platform, tabs)
        if target:
            bound[platform] = target

    save_workbench_targets({**existing, **bound})
    return {**existing, **bound}


def open_missing_platform_tabs(platforms):
    ...
    run_live_cdp("eval", base_target, js)
    time.sleep(1)
...
def main():
    ...
    if not args.no_auto_open:
        opened = open_missing_platform_tabs(platforms)
        if opened:
    ...
    for index, article_path in enumerate(article_paths, start=1):
        ...
        for platform in platforms:
            result = run_platform(platform, str(article_path), args.mode)
